...
class pom3_requirements:
    def __init__(requirements, decisions):
        ...
        for i in range(requirements.count):
            requirements.heap.add_root(
                Requirement((decisions.size + 1) * random_cost(),
                            random_value()),
                'Base Req #' + str('%.3d' % (i + 1)))
            ...
            if (rand <= 15):
                ...
                if ((i + 1) < len(requirements.heap.tree)):
                    ...
                    if adderDie <= decisions.interdependency:
                        requirements.add_dependency(requirements.heap.tree[i],
                                                    req_node)
    ...
    def add_children(self, num, parent, level):
        for c in range(num):
            parent.add_child(
                Requirement(random_cost(), random_value()), "+" * level +
                'Child-' + parent.key[0] + parent.key[len(parent.key) - 3] +
                parent.key[len(parent.key) - 2] +
                parent.key[len(parent.key) - 1] + ' #' + str('%.3d' % (c + 1)),
                level)
    ...
    def recursive_dep_adder(self, parent, rootIndex, level):
        ...
        if (len(parent.children) > 0
                and ((rootIndex + 1) < len(self.heap.tree))):
            if (level <= self.heap.tree[rootIndex + 1].max_depth()):
                ...
                if (rand <= odds[oddsInd]):
                    ...
                    levelNodes = self.heap.get_level(
                        self.heap.tree[rootIndex + 1], level)
